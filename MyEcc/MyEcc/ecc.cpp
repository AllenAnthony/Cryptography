...
{  /* Nyberg Rueppel */
   ...
   group = EC_GROUP_new(EC_GFp_mont_method()); /* 或EC_GFp_simple_method(); mont: BN_mod_mul_montgomery *///创建一个群
   ctx = BN_CTX_new();
   EC_GROUP_set_curve_GFp(group, p, a, b, ctx);//设置群参数

   G = EC_POINT_new(group);
   EC_POINT_set_affine_coordinates_GFp(group, G, gx, gy, ctx);//设置G的坐标
   EC_GROUP_set_generator(group, G, n, BN_value_one()); /* 基点=G, G的阶=n, 余因子=1 */
   dump_ecc(group);

   R = EC_POINT_new(group);
   EC_POINT_mul(group, R, d, NULL, NULL, ctx); /* 公钥R = d*G; d是私钥 *///生成公钥
   EC_POINT_get_affine_coordinates_GFp(group, R, tx, ty, ctx); /* tx = (d*G).x *///获得R的坐标，复制给tx, ty
   ...
   EC_POINT_mul(group, T, k, NULL, NULL, ctx); /* T = k*G *///获得r

   EC_POINT_get_affine_coordinates_GFp(group, T, tx, ty, ctx); /* tx = (k*G).x *///把r的坐标赋给tx, ty
   BN_mod(tx, tx, n, ctx);  /* tx = (k*G).x mod n */
   BN_bin2bn(plaintext, BN_num_bytes(n), m); /* m = plaintext *///把m明文根据二进制转换成一个大数
...
void ecc_encrypt_decrypt(void)
{
   ...
   EC_POINT_set_compressed_coordinates_GFp(group, T, r, 0, ctx); /* T=k*G;
                                                                    其中T的x坐标=r,
                                                                    y坐标自动计算 */
   EC_POINT_mul(group, T, NULL, T, d, ctx); /* T = d*k*G *///T就是红色的r,一个点
